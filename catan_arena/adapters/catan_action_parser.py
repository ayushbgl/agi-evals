...
from catan_arena.adapters.base import ActionParser, ActionParseError
...
class CatanActionParser(ActionParser):
    ...
    def parse(
        ...
        if parsed is None:
            if self.fallback_to_random and valid_actions:
                # Return random valid action as fallback
    ...
    def _extract_json(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        # Try 1: JSON in markdown code block
        ...
        # Try 2: Find any JSON object with action_type
        json_obj_pattern = r'\{[^{}]*"action_type"\s*:\s*"[^"]+?"[^{}]*\}'
        ...
        # Try 3: Find JSON object that might span multiple lines
        try:
    ...
    def _values_fuzzy_match(
                ...
                if len(parsed_value) >= 1:
                    if isinstance(parsed_value[0], (list, tuple)):
                        ...
                        if len(parsed_value) >= 2:
                            parsed_victim = parsed_value[1]
                    else:
                        # Might be flat coordinate
                ...
                if isinstance(valid_value, (list, tuple)) and len(valid_value) >= 1:
                    ...
                    if isinstance(valid_value[0], (list, tuple)):
                        ...
                        if len(valid_value) >= 2:
                            valid_victim = valid_value[1]

                    # Match if coordinates match (victim can differ)
        ...
        if action_type == "MARITIME_TRADE":
            if isinstance(parsed_value, (list, tuple)) and isinstance(valid_value, (list, tuple)):
                # Both should be 5-element tuples
